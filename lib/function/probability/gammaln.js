...
module.exports = function(math, config) {
  ...
  /**
   * Logarithm of the multivariate Gamma function for real, positive variables
   ...
   * @return {Number | Array | Matrix}  The logarathim of `gamma(x)`.
   */
  math.gammalnd = function gammalnd(x, d) {
    if (arguments.length != 2) {
        throw new math.error.ArgumentsError('gammalnd', arguments.length, 2);
    }
    if (isNumber(x))
    {
        var lnPI = 1.14472988584940;
        var r = d*(d-1)/4.*lnPI;
        for(var i = 0; i < d; i++) r += math.gammaln(x - 0.5*i);
    }
    if (isCollection(x))
    {
        return collection.deepMap(x, function(x) { gammalnd(x, d); });
    }
    return r;
  };
